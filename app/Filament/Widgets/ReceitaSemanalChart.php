...
class ReceitaSemanalChart extends ChartWidget
{
    ...
    {
        return 'Faturamento — Últimos 14 dias';
    }
    ...
    protected function getData(): array
    {
        ...
        return Cache::remember("chart_receita_data_{$tenantId}", 300, function () {
            $dados = Agendamento::whereIn('status', ['confirmado', 'concluido'])
                ...
                ->select([DB::raw('DATE(data_hora) as dia'), DB::raw('COUNT(*) as total'), DB::raw('COALESCE(SUM(COALESCE(agendamentos.valor_total, servicos.preco)), 0) as receita')])
