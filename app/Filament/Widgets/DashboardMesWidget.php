...
class DashboardMesWidget extends StatsOverviewWidget
{
    ...
    protected function getStats(): array
    {
        ...
        $data = Cache::remember("dashboard_mes_data_{$tenantId}", 300, function () {
            ...
            $receitaMesAnt = $agsMesAnt->sum(fn ($a) => $a->valor_total ?? $a->servico?->preco ?? 0);
            ...
            foreach ($agsMes->groupBy('profissional_id') as $pid => $profAgs) {
                ...
                $comissao += round($profAgs->sum(fn ($a) => $a->valor_total ?? $a->servico?->preco ?? 0) * (($prof?->comissao_percentual ?? 0) / 100), 2);
            }
            ...
            $sparkSemanal = Agendamento::whereIn('status', ['confirmado', 'concluido'])
                ...
                ->select([DB::raw('WEEK(data_hora) as semana'), DB::raw('COALESCE(SUM(COALESCE(agendamentos.valor_total, servicos.preco)),0) as receita')])
